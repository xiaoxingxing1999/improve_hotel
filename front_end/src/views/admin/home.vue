  ...
  <a-layout id="components-layout-demo-side" style="min-height: 100vh">
    <a-layout-sider v-model="collapsed" collapsible :width="150">
      <a-menu theme="dark" :selected-keys="current" mode="inline" style="margin-top:50px">
        <a-menu-item key="1" @click="jumpToHotelAndManager">
          <a-icon type="home" />
          <span>酒店及人员</span>
        </a-menu-item>
        <a-menu-item key="2" @click="jumpToHotelEdit">
          <a-icon type="info-circle" />
          <span>酒店信息修改</span>
        </a-menu-item>
        <a-menu-item key="3" @click="jumpToUserInfo">
          <a-icon type="user" />
          <span>个人信息</span>
        </a-menu-item>
      </a-menu>
...
export default {
  ...
  computed: {
    ...
    current: function() {
      let res = ["1"];
      if (this.$route.path === "/admin/hotelEdit") {
        res = ["2"];
      } else if (this.$route.path === "/admin/info") {
        res = ["3"];
      }
      return res;
    }
  ...
  methods: {
    ...
    jumpToHotelAndManager() {
      router.push("/admin/hotelAndManager");
    },
    jumpToHotelEdit() {
      router.push("/admin/hotelEdit");
    },
    jumpToUserInfo() {
